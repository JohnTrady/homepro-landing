(() => {
    ...
    window.onload = function() {
        const documentActions = e => {
            if (e.target.closest(".button-show-more")) {
                firstItem = document.querySelectorAll(".item-blog").length;
                lastItem = firstItem + 3;
                initBlog(data, firstItem, lastItem);
                e.preventDefault();
            }
        };
        ...
        const hideShowMore = () => {
            ...
            currentItems < dataItemsLenght ? showMoreButton.hidden = false : showMoreButton.hidden = true;
        };
        if (blogItems) loadBlogItems();
        const buildBlogItem = item => {
            let itemBlogTemplate = ``;
            let itemBlogTemplateBody = ``;
            itemBlogTemplate += `<article class="blog__item item-blog">`;
            item.image ? itemBlogTemplate += `\n        <a href="${item.url}" class="item-blog__image item-blog__image-ibg">\n        <img src="${item.image}" alt="Thumbnail">\n        </a>` : null;
            itemBlogTemplateBody += `\n\n        <div class="item-blog__body">\n        <div class="item-blog__date">${item.date}</div>\n        <h4 class="item-blog__title"><a href="${item.url}">${item.title}</a></h4>\n        <div class="item-blog__text">\n         ${item.text}\n        </div>\n       </div>\n         `;
            itemBlogTemplate += itemBlogTemplateBody;
            if (item.tags) {
                itemBlogTemplate += `  <div class="item-blog__tegs">`;
                for (const tag in item.tags) itemBlogTemplate += ` <a href="${item.tags[tag]}" class="item-blog__teg">${tag}</a>`;
                itemBlogTemplate += `</div>`;
            }
            itemBlogTemplate += `</article>`;
            blogItems.insertAdjacentHTML("beforeend", itemBlogTemplate);
        };
        const initBlog = (data, firstItem, lastItem) => {
